    ...
    public:
        ...
        void sortFilesInFolder(int flag) {
            if(flag == FLAG_SIZE) { // bubble sort 
                ...
                while (current != NULL) {
                    File *i = current; 
                    while (i->nextFile != NULL) {
                        if(i->nextFile->data->size < current->data->size) {
                            File *target = i->nextFile; // Identify the need-to-find node
                            if(current == this->firstFile) {
                                i->nextFile = target->nextFile; 
            
                                target->nextFile = current->nextFile;
                                this->firstFile = target; 
                                continue;   
                            }
                            prev->nextFile = current->nextFile; // Shutdown the current node out of list  
                            i->nextFile = target->nextFile; // Shutdown the current node out of list 

                            // Re-add those shutdown nodes
                            current->nextFile = target->nextFile; 
                            i->nextFile = current; 

                            target->nextFile = prev->nextFile; 
                            prev->nextFile = target; 
                        } 
                    }
                    // Next step going
                    if (prev == NULL) {
                        prev = this->firstFile;
                        current = prev->nextFile;
                    } 
                    else {
                        current = prev->nextFile->nextFile;
                        prev = prev->nextFile;
                    }
